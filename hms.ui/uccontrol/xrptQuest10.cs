...
namespace Hms.Ui
{
    public partial class xrptQuest10 : DevExpress.XtraReports.UI.XtraReport
    {
        public xrptQuest10(EntityQnRecord qnRecord)
        {
            InitializeComponent();
            ...
            try
            {
                if (qnRecord != null)
                {
                    if (!string.IsNullOrEmpty(qnRecord.xmlData))
                    {
                        ...
                        xmlData = list[10].OuterXml;
                        dicData = Function.ReadXML(xmlData);
                    }
                }
                using (ProxyHms proxy = new ProxyHms())
                {
                    lstCtrlLocation = proxy.Service.GetQnCtrlLocation(lstQuest[9], qnRecord.qnId);
