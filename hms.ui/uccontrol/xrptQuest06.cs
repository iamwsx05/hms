...
namespace Hms.Ui
{
    public partial class xrptQuest06 : DevExpress.XtraReports.UI.XtraReport
    {
        public xrptQuest06(EntityQnRecord qnRecord)
        {
            InitializeComponent();
            ...
            try
            {
                if (qnRecord != null)
                {
                    if (!string.IsNullOrEmpty(qnRecord.xmlData))
                    {
                        ...
                        xmlData = list[6].OuterXml;
                        dicData = Function.ReadXML(xmlData);
                    }
                }
                using (ProxyHms proxy = new ProxyHms())
                {
                    lstCtrlLocation = proxy.Service.GetQnCtrlLocation(lstQuest[5], qnRecord.qnId);
