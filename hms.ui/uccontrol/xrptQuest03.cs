...
namespace Hms.Ui
{
    public partial class xrptQuest03 : DevExpress.XtraReports.UI.XtraReport
    {
        public xrptQuest03(EntityQnRecord qnRecord)
        {
            InitializeComponent();
            ...
            try
            {
                if (qnRecord != null)
                {
                    if (!string.IsNullOrEmpty(qnRecord.xmlData))
                    {
                        ...
                        xmlData = list[3].OuterXml;
                        dicData = Function.ReadXML(xmlData);
                    }
                }
                using (ProxyHms proxy = new ProxyHms())
                {
                    lstCtrlLocation = proxy.Service.GetQnCtrlLocation(lstQuest[2], qnRecord.qnId);
