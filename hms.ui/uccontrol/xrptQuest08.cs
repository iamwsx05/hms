...
namespace Hms.Ui
{
    public partial class xrptQuest08 : DevExpress.XtraReports.UI.XtraReport
    {
        public xrptQuest08(EntityQnRecord qnRecord)
        {
            InitializeComponent();
            ...
            try
            {
                if (qnRecord != null)
                {
                    if (!string.IsNullOrEmpty(qnRecord.xmlData))
                    {
                        ...
                        xmlData = list[8].OuterXml;
                        dicData = Function.ReadXML(xmlData);
                    }
                }
                using (ProxyHms proxy = new ProxyHms())
                {
                    lstCtrlLocation = proxy.Service.GetQnCtrlLocation(lstQuest[7], qnRecord.qnId);
